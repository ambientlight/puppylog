CREATE OR REPLACE PROCEDURE total_traffic_alert(job_id int, config jsonb) LANGUAGE PLPGSQL AS
$$
DECLARE
    hits INT;
    last_ts TIMESTAMP;
...
BEGIN
    ...
    BEGIN
        SELECT tbucket, total_logs INTO STRICT last_ts, hits from total_traffic WHERE tbucket > now() - interval_offset ORDER BY tbucket DESC LIMIT 1;
        IF hits > alert_level THEN
            INSERT INTO alerthistory(job_id, name, trigger_ts, alert_level, observed_level, metric) 
            VALUES (job_id, alert_name, last_ts, alert_level, hits, 'total_traffic');
...
SELECT add_job('total_traffic_alert', '1m', config => '{"alert_name":"TotalTrafficAlert", "offset": "1y", "level": 10}');