...
AS
SELECT {{{customSegmentExpression}}} time_bucket('{{period}}', ts) as tbucket, {{statistic}}({{property}}) observation FROM logrecords GROUP BY {{{customSegmentExpression}}} time_bucket('{{period}}', ts);
